  ...
  <main class="pb-24 mx-auto px-4 border-b border-gray-800">
    <div class="flex bg-gray-200 my-10 mx-auto p-5 w-[calc(100%-96)] rounded justify-center text-gray-900">
      <div class="flex flex-col xl:flex-row my-6">
        ...
        <div class="w-full mt-4 xl:mt-0 xl:mr-4">
          ...
          <div class="space-y-6 mt-4">
            ...
            <section v-if="$page.props.newsStories.length > 0" class="bg-white p-4 rounded-lg shadow">
              ...
              <div v-for="(story, index) in displayedNewsStories" :key="story.id"
                ...
                <div class="flex gap-2 items-start">
                  <SingleImage v-if="story.image" :image="story.image" :alt="story.title" :class="`w-12 h-12 min-w-12 min-h-12 object-cover shadow-lg rounded-lg`"/>
                  <div v-else class="w-12 h-12 min-w-12 min-h-12 bg-gray-300 shadow-lg rounded-lg flex items-center justify-center">
                    <i class="fas fa-image text-gray-500"></i>
                  </div>
                  <div>
                    <span>{{ story.title }}</span>
                    <span><ConvertDateTimeToTimeAgo :dateTime="story.published_at" :timezone="userStore.timezone" /></span>
                  </div>
                </div>
              </div>
              <div class="flex justify-center mt-4">
                <button v-if="showSeeMore" @click="showAllStories = true" class="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition duration-300">
            ...
            <section v-if="Object.keys($page.props.newsPerson.social_media).length" class="bg-white p-4 rounded-lg shadow">
              ...
              <div class="flex flex-wrap gap-4">
                <a v-for="(url, platform) in socialMediaLinks"
                   ...
                   :title="platform">
                  <font-awesome-icon :icon="['fab', socialMediaIcons[platform]]" class="text-2xl" v-if="platform !== 'substack'"/>
...
import { faFacebookF, faTwitter, faInstagram, faLinkedin, faSnapchat, faDiscord } from '@fortawesome/free-brands-svg-icons'
