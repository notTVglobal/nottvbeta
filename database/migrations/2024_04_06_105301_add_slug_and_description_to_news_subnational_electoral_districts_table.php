...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('news_subnational_electoral_districts', function (Blueprint $table) {
      ...
      $subNationalDistricts = \App\Models\NewsSubnationalElectoralDistrict::all();
      foreach ($subNationalDistricts as $district) {
        // Generate the initial slug from the district name
        $basicSlug = Str::slug($district->name);
        $slug = $basicSlug;

        // Check if the slug is already taken
        $existingSlugCount = \App\Models\NewsSubnationalElectoralDistrict::where('slug', $slug)->count();
